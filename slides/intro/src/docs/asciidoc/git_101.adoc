...
* *Master* holds released commits
* *Dev* contains unreleased commits  
* *Feature* contains feature commits
* *Hotfix* contains hotfix commits
...
* We want to keep history of hotfixes and branches  
...
* A new feature has been requested for a new shell function
* We want the new feature to not interefere with others
* Unfortunately, our new feature will produce an conflict

.swagger.sh
[source,bash]
--
include::./examples/101/src/c_2/swagger.sh[tag=newfunction] 
--
...
* We want to delete the branch after the merge
